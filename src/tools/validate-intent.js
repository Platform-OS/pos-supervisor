  ...
  inputSchema: {
    ...
    properties: {
      scaffold_output: {
        type: 'object',
        description:
          'Mode 1: pass the complete result object returned by the scaffold tool. Runs project-state checks (cross-references with existing project) and policy checks. Schema/domain checks are skipped — scaffold already generates correct paths.',
        properties: {
          files: {
            type: 'array',
            items: {
          ...
          creation_order: { type: 'array', items: { type: 'string' } },
          conflicts: {
            type: 'array',
            items: {
              type: 'object',
      ...
      intent: {
        type: 'object',
        description:
          'Mode 2: manually declared plan for hand-written files. Runs all validation layers: schema correctness, path/role rules, cross-reference with existing project, and architecture policy.',
        properties: {
          goal: {
            type: 'string',
            description: 'What this change is trying to accomplish, in plain language.',
          },
          changes: {
            type: 'array',
            description:
              'Array of file operations. Must be a JSON array — not a string. Each item describes one file: its path, role, action, and optional references to other files it depends on.',
            items: {
              type: 'object',
              properties: {
                path: {
                  type: 'string',
                  description: 'File path relative to project root. Must match the required pattern for the role.',
                },
                role: {
                  type: 'string',
                  enum: ['page', 'layout', 'partial', 'graphql', 'schema', 'command', 'query', 'translation', 'lib', 'asset', 'config', 'migration', 'module_override'],
                  description: [
                ...
                action: {
                  ...
                  description: 'Operation: create (new file), update (modify existing), delete (remove), move (rename/relocate — also provide move_to).',
                },
                move_to: {
                  type: 'string',
                ...
                references: {
                  type: 'object',
                  description:
                    'Optional but important: list files and resources this change depends on. Used to verify they exist in the project or are being created in this same plan.',
                  properties: {
                    partials:     { type: 'array', items: { type: 'string' }, description: 'Partial names rendered by this file (e.g., "blog_posts/form"). Strip app/views/partials/ prefix and .liquid extension.' },
                    graphql:      { type: 'array', items: { type: 'string' }, description: 'GraphQL operation names used (e.g., "blog_posts/search"). Strip app/graphql/ prefix and .graphql extension.' },
                    schemas:      { type: 'array', items: { type: 'string' }, description: 'Schema table names referenced (e.g., "blog_post"). Strip app/schema/ prefix and .yml extension.' },
                    translations: { type: 'array', items: { type: 'string' }, description: 'Translation keys used in dot notation (e.g., "app.blog_posts.list.title").' },
                    commands:     { type: 'array', items: { type: 'string' }, description: 'Full relative paths of commands called by this file (e.g., "app/lib/commands/blog_posts/create.liquid").' },
                    queries:      { type: 'array', items: { type: 'string' }, description: 'Full relative paths of queries called by this file (e.g., "app/lib/queries/blog_posts/search.liquid").' },
                  },
                },
              },
